...
const inp = 'w-full bg-slate-950/70 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 transition-colors placeholder-slate-600';
...
function NodeCard({ node, index, total, onEdit, onDelete, onDuplicate, onMoveUp, onMoveDown }) {
  ...
  return (
    <div
      className="group relative rounded-2xl border transition-all duration-200 cursor-pointer hover:shadow-lg"
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-3.5 pb-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 transition-colors opacity-0 group-hover:opacity-100"
            title="Edit node"
          >
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-7 z-30 w-40 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl overflow-hidden"
                onMouseLeave={() => setMenuOpen(false)}>
                <button onClick={() => { setMenuOpen(false); onEdit(node); }}
                  className="flex items-center gap-2 w-full px-3 py-2 text-xs text-slate-300 hover:bg-slate-800 transition-colors">
      ...
      <div className="px-4 pb-1">
        <p className="text-sm font-semibold text-white">{node.title || '(Untitled)'}</p>
      </div>

      {/* Content Preview */}
      {node.content && (
        <div className="mx-4 mb-2 px-3 py-2 rounded-lg bg-slate-950/50 border border-white/5">
          <p className="text-xs text-slate-300 line-clamp-2 font-mono leading-relaxed">{node.content}</p>
        </div>
      )}

      {/* Media URL */}
      {node.mediaUrl && (
        <div className="mx-4 mb-2 px-2 py-1 rounded bg-violet-500/10 border border-violet-500/20">
          <p className="text-[10px] text-violet-300 truncate">🖼 {node.mediaUrl}</p>
        </div>
      )}
      ...
      {node.buttons && node.buttons.length > 0 && (
        <div className="px-4 pb-2 flex flex-wrap gap-1.5">
          {node.buttons.map((b, i) => (
            <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] bg-slate-800 text-slate-300 border border-slate-700">
              {b.title || '(empty)'} {b.nextNodeId && <ArrowRight className="w-2.5 h-2.5 text-emerald-400" />} {b.nextNodeId ? <span className="font-mono text-emerald-400">{b.nextNodeId.slice(-8)}</span> : 'End'}
            </span>
          ))}
        </div>
      )}
      ...
      {/* Webhook URL */}
      {node.webhookUrl && (
        <div className="mx-4 mb-2 px-2 py-1 rounded bg-orange-500/10 border border-orange-500/20">
          <p className="text-[10px] text-orange-300 truncate">🌐 {node.webhookUrl}</p>
        </div>
      )}
      ...
      {node.nextNodeId && !['buttons', 'quick_reply', 'list', 'condition'].includes(node.type) && (
        <div className="px-4 pb-3 flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-slate-400 font-mono">{node.nextNodeId}</span>
        </div>
      )}

      {/* Double-click hint */}
      <div className="absolute bottom-2 right-3 text-[9px] text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity">
...
function AddNodePalette({ onAdd }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((p) => !p)}
        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-slate-700 text-slate-500 hover:border-emerald-500/50 hover:text-emerald-400 transition-all text-xs font-semibold"
      >
        <Plus className="w-4 h-4" /> Add Node
      </button>
      {open && (
        <div className="absolute left-0 right-0 mt-2 z-20 grid grid-cols-2 gap-1.5 p-2 rounded-xl bg-slate-900 border border-slate-700 shadow-2xl">
          {NODE_TYPES.map((t) => {
            const Icon = t.icon;
            return (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-colors hover:bg-slate-800 text-slate-300"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div className="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden">
              ...
              <div className="p-2 space-y-1.5 max-h-[70vh] overflow-y-auto">
                {loading ? (
                  ...
                  <div className="py-8 text-center text-xs text-slate-500">No flows yet. Create one to begin.</div>
                ) : (
                  flows.map((flow) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs font-semibold text-white truncate">{flow.name}</p>
                          <span className={`shrink-0 w-2 h-2 rounded-full ${flow.isActive ? 'bg-emerald-400' : 'bg-slate-600'}`} title={flow.isActive ? 'Active' : 'Inactive'} />
                        </div>
          ...
          <div className="lg:col-span-6">
            {editedFlow ? (
              <div className="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden">
                ...
                <div className="px-4 py-3 border-b border-slate-800 bg-slate-950/50 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-white truncate max-w-[180px]">{editedFlow.name}</span>
                    {isDirty && (
                  ...
                  <div className="flex items-center gap-2">
                    {isDirty && (
                      <button onClick={handleCancel}
                        className="px-3 py-1.5 text-xs rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-colors">
                        Cancel
                      </button>
                    )}
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-xl font-semibold transition-all ${
                        isDirty ? 'bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-md shadow-emerald-500/20' : 'bg-slate-800 text-slate-500 cursor-not-allowed'
                      }`}
                    >
                ...
                {saveError && (
                  <div className="mx-4 mt-3 px-3 py-2 rounded-lg bg-rose-500/10 border border-rose-500/20 flex items-center gap-2 text-xs text-rose-300">
                    <AlertTriangle className="w-3.5 h-3.5 shrink-0" />{saveError}
                  </div>
                )}

                {/* Nodes Canvas */}
                <div className="p-4 space-y-0.5 min-h-[400px] max-h-[70vh] overflow-y-auto">
              ...
              <div className="rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center min-h-[400px]">
                <div className="text-center text-slate-500">
                  <Bot className="w-12 h-12 mx-auto mb-3 opacity-20" />
                  <p className="text-sm font-medium">Select a flow from the sidebar</p>
                  <p className="text-xs mt-1">or create a new one to start building</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-3 space-y-3">
            {editedFlow ? (
              <>
                ...
                <div className="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-800 bg-slate-950/40">
                    <span className="text-xs font-bold text-slate-300 uppercase tracking-widest">Flow Settings</span>
                  </div>
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${editedFlow.isActive ? 'bg-emerald-500' : 'bg-slate-700'}`}
                      >
                ...
                <div className="rounded-2xl bg-rose-500/5 border border-rose-500/20 p-4">
                  <span className={sectionLabel + ' text-rose-400'}>Danger Zone</span>
                  <button
                    onClick={() => handleDeleteFlow(editedFlow._id)}
                    className="mt-2 w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 border border-rose-500/30 text-rose-300 text-xs font-semibold transition-colors"
                  >
              ...
              <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6 text-center text-slate-600 text-xs">
                No flow selected
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-slate-950/50">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                <Bot className="w-4 h-4 text-emerald-400" /> Create New Bot Flow
              </h3>
              <button onClick={() => setIsCreateOpen(false)} className="p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
