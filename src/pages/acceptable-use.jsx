...
export default function AcceptableUsePolicy() {
  return (
    <LegalLayout title="Acceptable Use Policy" description={`${COMPANY.name} Acceptable Use Policy and WhatsApp Cloud API Messaging Guidelines.`}>
      <div className="space-y-6">
        ...
        <div className="prose prose-invert max-w-none text-xs text-slate-300 space-y-4">
          <section className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
            ...
            <p>
              All WhatsApp messaging sent via {COMPANY.name} ({COMPANY.website}) must be explicitly consented to by the receiving end-user. Sending bulk unsolicited messages, buying contact lists, or circumventing user opt-outs is strictly prohibited and subject to automated account termination.
            </p>
          </section>
          ...
          <section className="p-6 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
            ...
            <p>
              Tenants must comply with Meta Commerce &amp; Business Policies. Content involving illegal products, deceptive claims, malware, or harassment will be reported to Meta security teams. Contact compliance at <code>{COMPANY.contactEmail}</code>.
            </p>
          </section>
        </div>
      </div>
